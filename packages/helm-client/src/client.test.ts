...
async function connect(client: HelmClient, sockets: FakeSocket[]): Promise<FakeSocket> {
...
describe("HelmClient", () => {
  ...
  test("surfaces daemon errors", async () => {
    ...
    await expect(response).rejects.toBeInstanceOf(HelmRpcError);
  });
  ...
  test("disconnect rejects an in-flight handshake and permits reconnecting", async () => {
    ...
    const firstConnection = client.connect();
    client.disconnect();
    await expect(firstConnection).rejects.toThrow("Helm client disconnected");
  ...
  test("disconnected requests reject instead of throwing synchronously", async () => {
    ...
    await expect(request).rejects.toThrow("Helm daemon is disconnected");
  });
  ...
  test("disconnected notifications reject instead of throwing synchronously", async () => {
    ...
    await expect(notification).rejects.toThrow("Helm daemon is disconnected");
  });
...
test("daemonUrl pins the versioned endpoint", () => {
  ...
  expect(daemonUrl("wss://helm.example.test")).toBe("wss://helm.example.test/v1");
});

describe("HelmClient connection failures", () => {
  async function failure(setup: (socket: FakeSocket) => void): Promise<HelmConnectionError> {
    const { client, sockets } = fixture();
    const connected = client.connect();
    setup(sockets[0]!);
    const error = await connected.catch((cause: unknown) => cause);
    expect(error).toBeInstanceOf(HelmConnectionError);
    return error as HelmConnectionError;
  }
  ...
  test("times out a handshake the daemon never answers", async () => {
    ...
    const error = (await client.connect().catch((cause: unknown) => cause)) as HelmConnectionError;
  ...
  test("lets a request shorten its own timeout", async () => {
    ...
    ).rejects.toThrow("timed out waiting for Helm daemon");
  });
  ...
  test("settles requests and clears the socket before listeners hear a remote close", async () => {
    ...
    const error = (await request.catch((cause: unknown) => cause)) as HelmConnectionError;

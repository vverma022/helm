...
use helm_protocol::{DAEMON_TOKEN_ENV, DaemonReady, PROTOCOL_VERSION};

fn main() -> anyhow::Result<()> {
    let arguments = Arguments::parse(std::env::args().skip(1))?;
    let token =
        std::env::var(DAEMON_TOKEN_ENV).context("Helm daemon authentication token is missing")?;
    ...
    let listener = TcpListener::bind(&arguments.bind)
        .with_context(|| format!("could not bind Helm daemon to {}", arguments.bind))?;
    ...
    let task_path = helm_core::persistence::StateStore::default_path();
    let settings = helm_core::DaemonSettingsStore::open_with_legacy(
        helm_core::DaemonSettings::default_path(),
        [task_path.with_file_name("settings.json")],
    )
    .context("could not load daemon settings")?;
    let task_store = helm_core::persistence::StateStore::daemon(task_path);
    helm_core::serve(
        listener,
        token,
        Arc::new(helm_core::daemon::HelmBackend::new(settings, task_store)?),
        shutdown,
        helm_core::ServerOptions {
...
mod tests {
    ...
    fn parses_repeated_browser_origin_allowlist_entries() {
        ...
        assert_eq!(
            ...
            ["https://app.helm.test", "http://localhost:3000"]
        );

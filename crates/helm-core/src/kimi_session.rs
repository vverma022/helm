...
//! empty answer and calls it a success. Reading the wire log is what lets Helm
...
/// permission dialogs just by opening Helm's Resume picker.
...
/// hash Helm cannot reproduce, so the session id is matched by scanning.

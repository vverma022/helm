...
//! random fallback. Helm mirrors that layout beneath `~/.helm/projects` so
...
fn workspace_root_slot() -> &'static RwLock<Option<PathBuf>> {
    ...
    ROOT.get_or_init(|| {
        RwLock::new(dirs::home_dir().map(|home| home.join(".helm").join("projects")))
    })
}

pub fn set_workspace_root(root: Option<PathBuf>) {
    helm_protocol::projectless::set_workspace_root(root.clone());
...
/// Existing builds created dated workspaces directly under `~/.helm`; keep
...
pub fn create_workspace(prompt: Option<&str>) -> io::Result<Workspace> {
    let root = workspace_root().ok_or_else(|| {
        io::Error::new(
            ...
            "could not locate the home directory for ~/.helm/projects",
        )
    })?;
    create_workspace_in(&root, Local::now().date_naive(), None, prompt)
}

/// Move one old dated workspace from `~/.helm/<date>/<slug>` into
/// `~/.helm/projects/<date>/<slug>` without copying its contents through the
/// client. The oldest layout used `~/.helm` itself; that path contains Helm's
...
mod tests {
    ...
    fn test_root() -> PathBuf {
        std::env::temp_dir().join(format!("helm-projectless-{}", Uuid::new_v4()))
    }

...
const NATIVE_CLIENT_HEADER: &str = "x-helm-client";
...
pub struct ServerOptions {
    ...
    /// do not; React Native does and identifies itself with `x-helm-client`.
    ...
    fn dispatch_runtime(
        ...
        loop {
            ...
            let worker = std::thread::Builder::new()
                .name(format!("helm-daemon-runtime-{session_id}"))
                .spawn(move || {
                    run_runtime_mailbox(
                        session_id,
                        mailbox_id,
...
pub fn serve(
    ...
    listener
        ...
        .context("could not configure Helm daemon listener")?;
        ...
        match listener.accept() {
            Ok((stream, _)) => {
                ...
                std::thread::Builder::new()
                    .name("helm-daemon-connection".into())
                    .spawn(move || {
                        let _connection_permit = connection_permit;
                        if let Err(error) =
                            handle_connection(stream, &token, dispatcher, hub, shutdown, &options)
                        {
                            eprintln!("helm-daemon connection ended: {error:#}");
                        }
                    })
                    .context("could not start Helm daemon connection thread")?;
            }
            Err(error) if error.kind() == io::ErrorKind::WouldBlock => {
                std::thread::sleep(ACCEPT_POLL_INTERVAL);
            }
            Err(error) if error.kind() == io::ErrorKind::Interrupted => {}
            Err(error) => return Err(error).context("Helm daemon listener failed"),
        }
    }
    backend.shutdown();
    Ok(())
    ...
    'connection: while !shutdown.load(Ordering::Acquire) {
        ...
        match socket.read() {
            Ok(Message::Text(text)) => match serde_json::from_str(text.as_ref()) {
                ...
                Err(error) => {
                    eprintln!("helm-daemon ignored invalid message: {error}");
                }
            },
            ...
            Err(error) => return Err(error).context("Helm daemon WebSocket failed"),
        }
    }
...
mod tests {
    ...
    use helm_client::{DaemonClient, DaemonSupervisor};
        ...
        fn handle(&self, request: Request, _events: EventSink) -> anyhow::Result<ResponsePayload> {
            match request.command {
                ...
                Command::LoadTaskState => Ok(ResponsePayload::TaskState {
                    ...
                    projectless_root: Some(PathBuf::from("/tmp/.helm/projects")),
                }),
                _ => Ok(ResponsePayload::Ack),
            }
        }
    ...
    fn stale_projection_cannot_resurrect_a_removed_session() {
        let root = std::env::temp_dir().join(format!("helm-remove-race-{}", Uuid::new_v4()));
    ...
    fn websocket_terminal_round_trip_streams_input_and_output() {
        ...
        client
            .request(
                ...
                terminal_id,
                Command::WriteTerminal {
                    data: b"helm-terminal-round-trip\r".to_vec(),
                },
            )
            .unwrap();
    ...
    fn browser_origins_are_denied_unless_explicitly_allowed() {
        let request = HandshakeRequest::builder()
            .uri("/v1")
            .header(ORIGIN, "https://app.helm.test")
            .body(())
            .unwrap();
        ...
        let allowed = HashSet::from(["https://app.helm.test".to_owned()]);

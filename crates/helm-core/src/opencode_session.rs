...
/// sessions, so a catalog launched from Helm's isolated temp directory saw
...
fn retained_turn_count(total_turns: usize, turns_to_remove: usize) -> anyhow::Result<usize> {
    total_turns.checked_sub(turns_to_remove).ok_or_else(|| {
        anyhow!(
            "OpenCode has only {total_turns} native turns, but Helm needs to remove {turns_to_remove}"
        )
    })
}
...
fn fork_message_id(message_ids: &[String], retained_turns: usize) -> anyhow::Result<Option<&str>> {
    if retained_turns > message_ids.len() {
        bail!(
            "OpenCode has only {} native turns, but Helm needs {retained_turns}",
            message_ids.len()
        );
    }
...
mod tests {
    ...
    fn global_session_list_maps_root_sessions_across_projects() {
        let catalog_root = std::env::temp_dir().join("helm-opencode-session-catalog");
        let helm_directory = catalog_root.join("dev").join("helm");
        let response = json!([
            {
                "id": "ses_helm",
                "title": "Review and merge Helm PR #113",
                "directory": helm_directory,
                "time": { "created": 1_787_000_000_123_u64, "updated": 1_787_000_100_999_u64 },
                "project": { "id": "prj_helm", "worktree": helm_directory }
            },
            {
                "id": " ses_untitled ",
        ...
        assert_eq!(
            ...
            ProviderResumeCursor::OpenCode {
                session_id: "ses_helm".into()
            }
        );
        assert_eq!(sessions[0].title, "Review and merge Helm PR #113");
        assert_eq!(sessions[0].cwd, helm_directory);
    ...
    /// submitted after Helm has relaunched. The source session is supplied by
    ...
    #[test]
    #[ignore = "requires an installed opencode and HELM_OPENCODE_TEST_SESSION_ID"]
    fn forks_away_a_real_single_turn_session() {
        let binary =
            crate::command_env::find_executable("opencode").expect("opencode is not installed");
        let session_id = std::env::var("HELM_OPENCODE_TEST_SESSION_ID")
            .expect("set HELM_OPENCODE_TEST_SESSION_ID to a completed one-turn session");

...
//! permission requests the user can actually be asked. Helm already started
...
impl OpenCodeDriver {
    pub fn start(options: DriverStartOptions, events: DriverEventSender) -> anyhow::Result<Self> {
        ...
        // Helm must therefore install the selected access policy on this
        ...
        thread::Builder::new()
            .name("helm-opencode-usage-metadata".into())
            .spawn(move || {
        ...
        thread::Builder::new()
            .name("helm-opencode-events".into())
            .spawn(move || {
                ...
                match open_event_stream(stream_port, "/event", &stream_control) {
                    Ok(Some(stream)) => {
                        ...
                        // lost when Helm reconnects after it was asked. Events
        ...
        thread::Builder::new()
            .name("helm-opencode-driver".into())
            .spawn(move || {
...
fn rehydrate_pending_permissions(
    ...
    // `session.idle` settles Helm's persisted running turn exactly once.
...
fn request_permission(
    ...
    // so Helm sends only one-shot provider replies and retains durable choices
...
mod tests {
    ...
    /// `cargo test --bin helm opencode_session_against_a_real_server -- --ignored`.
    #[test]

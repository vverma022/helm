//! Provider backend and driver-event wire translation for `helm-daemon`.
...
use helm_protocol::provider_session::{ProviderSessionFork, ProviderSessionForkRequest};
...
impl HelmBackend {
    pub fn new(settings: DaemonSettingsStore, task_store: StateStore) -> anyhow::Result<Self> {
        let mut task_state = task_store
            .load()
            .context("could not load Helm task database")?;
...
fn migrate_projectless_state(
    ...
    for index in indices {
        ...
        let workspace = crate::projectless::migrate_workspace(&old_path).with_context(|| {
            format!(
                "could not move projectless workspace {} under ~/.helm/projects",
                old_path.display()
            )
        })?;
        ...
        match request.command {
            ...
            Command::Start { options } => {
                ...
                std::thread::Builder::new()
                    .name(format!("helm-daemon-events-{session_id}"))
                    .spawn(move || {
...
mod tests {
    ...
    fn client_projection_cannot_replace_a_daemon_checkpoint() {
        ...
        let checkpoint = Checkpoint {
            turn_count: 1,
            git_ref: "refs/helm/canonical".into(),
        ...
        incoming.turns[0].checkpoint = Some(Checkpoint {
            git_ref: "refs/helm/stale-client".into(),
            ..checkpoint.clone()
        });

...
// Rust destructors. Keep one pipe open in Helm and let this wrapper terminate
...
impl DeepSeekServer {
    ...
    fn start_with_dsh_home(binary: &Path, dsh_home: Option<&Path>) -> anyhow::Result<Self> {
        ...
        let mut command = {
            ...
            command
                .args(["-c", DSH_GUARDIAN_SCRIPT, "helm-dsh-guardian"])
                .arg(binary)
        ...
        thread::Builder::new()
            .name("helm-deepseek-stdout".into())
            .spawn(move || {
        ...
        thread::Builder::new()
            .name("helm-deepseek-stderr".into())
            .spawn(move || {
        ...
        for (name, path) in [("mux", "/api/events.mux"), ("host", "/api/events.host")] {
            ...
            thread::Builder::new()
                .name(format!("helm-deepseek-{name}-events"))
        ...
        thread::Builder::new()
            .name("helm-deepseek-process-monitor".into())
            .spawn(move || {
                loop {
                    ...
                    match status {
                        Ok(Some(status)) => {
                            monitor_events.publish(json!({
                                "type": "server-request",
                                "rpcId": format!("helm-process-{}", Uuid::new_v4()),
                                "method": "helm/process-exited",
                                "payload": {
                                    "type": "helm/process-exited",
    ...
    pub(crate) fn rpc(&self, method: &str, payload: Value) -> anyhow::Result<Value> {
        let rpc_id = format!("helm-{}", Uuid::new_v4());
        let body = json!({
            "type": "client-request",
            "rpcId": rpc_id,
            "method": method,
...
fn stream_error(message: String) -> Value {
    json!({
        "type": "server-request",
        "rpcId": format!("helm-stream-{}", Uuid::new_v4()),
        "method": "stream/error",
        "payload": {
            "type": "stream/error",
...
mod tests {
    ...
    /// Exercises Helm's HTTP envelope, WebSocket handshakes, and process-tree
    ...
    fn installed_harness_host_answers_rpc() {
        ...
        #[cfg(unix)]
        {
            // Simulate an abruptly terminated Helm process: the OS closes its
    ...
    fn installed_harness_session_contracts_round_trip() {
        ...
        let binary =
            crate::command_env::find_executable("dsh").expect("DeepSeek Harness is not installed");
        let root =
            TempHarnessHome(std::env::temp_dir().join(format!("helm-dsh-test-{}", Uuid::new_v4())));
        std::fs::create_dir_all(&root.0).unwrap();
        {
            let server = DeepSeekServer::start_with_dsh_home(&binary, Some(&root.0))
                .expect("Harness Host should start");
            let session_id = format!("helm-test-{}", Uuid::new_v4());
            ...
            let created = server
                ...
                .expect("session.create should accept Helm's payload");
            assert_eq!(
            ...
            let history = server
                ...
                .expect("session.history should accept Helm's payload");

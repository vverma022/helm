...
//! worktree beneath `~/.helm/worktrees`, then every workspace consumer uses
...
pub use helm_protocol::git::CreatedWorktree;
...
pub fn create(
    ...
    let root = dirs::home_dir()
        .ok_or_else(|| anyhow!("could not locate the home directory for ~/.helm/worktrees"))?
        .join(".helm/worktrees");
    create_in(
        project_path,
        &root,
...
fn create_in(
    ...
    let branch = format!("helm/{fallback}");
...
mod tests {
    ...
    fn creates_isolated_worktrees_from_the_default_branch() {
        let root = std::env::temp_dir().join(format!("helm-worktree-test-{}", Uuid::new_v4()));
        ...
        .unwrap();
        assert_eq!(first.branch, "helm/build-a-project-selector");
        assert_eq!(
        ...
        .unwrap();
        assert_eq!(second.branch, "helm/build-a-project-selector-2");

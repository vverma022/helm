...
/// conversation on the next Helm prompt.
...
fn list_provider_sessions_in(
    ...
    // from this index, which naturally keeps Helm-created sessions out of the
    // terminal resume picker even after their Helm task has been deleted.
...
fn fork_session_at_in(
    ...
    output.push(json!({
        "type": "custom-title",
        "sessionId": forked_session_id,
        "customTitle": if title.trim().is_empty() { "Helm rewind" } else { title.trim() },
        "uuid": Uuid::new_v4().to_string(),
        "timestamp": now,
    }));
...
mod tests {
    ...
    fn fixture() -> (PathBuf, PathBuf) {
        let root = std::env::temp_dir().join(format!("helm-claude-session-{}", Uuid::new_v4()));

...
describe('isDaemonDisconnectError', () => {
  test('recognizes the client transport messages emitted during a dropped link', () => {
    expect(isDaemonDisconnectError(new Error('Helm daemon disconnected'))).toBe(true);
    expect(isDaemonDisconnectError('Helm daemon is disconnected')).toBe(true);
    expect(isDaemonDisconnectError('Helm client disconnected')).toBe(true);
  });

  test('does not swallow a real daemon or agent error', () => {
    expect(isDaemonDisconnectError('timed out waiting for Helm daemon')).toBe(false);
...
describe('clearDaemonDisconnectErrors', () => {
  test('removes only stale transport errors after reconnecting', () => {
    const errors = {
      disconnected: 'Helm daemon disconnected',

...
Browser client for an existing Helm daemon. The Cloudflare Worker serves the
TanStack Start application only; it does not start, proxy, or store credentials
for a daemon.

```sh
bun --filter @helm/web dev
```
...
```sh
HELM_DAEMON_TOKEN=replace-me cargo run -p helm-daemon --bin helm-daemon -- \
  --bind 127.0.0.1:34123 \
  --allow-origin http://localhost:3001
```

Helm Desktop can expose the daemon it manages from Settings → Daemon, where you
...
```sh
bun --filter @helm/web build
bun --filter @helm/web deploy
```

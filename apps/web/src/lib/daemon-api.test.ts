...
describe('turn checkpoints', () => {
  test('captures the immutable starting ref on the daemon host', async () => {
    ...
    await expect(captureTurnStart(client, '/srv/helm', 'session', 2)).resolves.toBeUndefined()
    expect(command).toEqual({
      type: 'workspace',
      operation: {
  ...
  test('returns the ending checkpoint captured by the daemon', async () => {
    ...
    const checkpoint = {
      turn_count: 2,
      git_ref: 'refs/helm/session-session-turn-2',
    ...
    await expect(captureTurnCheckpoint(client, '/srv/helm', 'session', 2))
...
describe('probeProvider', () => {
  test('can detect an executable without starting model or version discovery', async () => {
    ...
    await expect(probeProvider(client, 'codex', settings, {
      ...
      probeVersion: false,
    })).resolves.toMatchObject({ installed: true, path: '/opt/helm/codex' })
    expect(command).toEqual({
      type: 'probeProvider',
      provider: 'codex',
      binaryOverride: null,
...
describe('writeWorkspaceTextFile', () => {
  test('writes the edited contents through the daemon workspace API', async () => {
    ...
    await expect(
      writeWorkspaceTextFile(client, '/srv/helm', 'src/app.ts', 'export const ready = true\n'),
...
describe('createProject', () => {
  test('normalizes a remote absolute path without collapsing the root', () => {
    ...
    expect(createProject('/srv/helm/').path).toBe('/srv/helm')
    expect(createProject('/srv/helm/').name).toBe('helm')
  })
...
describe('persistProject', () => {
  test('adds a daemon-host project without creating a session', async () => {
    ...
    const candidate = project('new', 'helm', '/srv/helm')
      ...
      request: async (command: unknown) => {
        ...
        if ((command as { type: string }).type === 'loadTaskState') {
          return {
            ...
            sessions: [{ id: 'session' }],
            defaultCwd: '/srv',
            projectlessRoot: '/srv/.helm/projects',
          }
        }
  ...
  test('reuses a project already persisted for the same daemon path', async () => {
    const existing = project('existing', 'helm', '/srv/helm')
    const commands: unknown[] = []
    const client = {
      request: async (command: unknown) => {
        commands.push(command)
    ...
    const result = await persistProject(client, project('duplicate', 'helm', '/srv/helm'))
...
describe('provider session resume', () => {
  ...
  test('lists the catalog and loads only the selected history', async () => {
    ...
    expect(commands).toEqual([
      ...
      { type: 'loadProviderSession', cursor: summary.cursor, cwd: '/srv/helm' },
    ])
  })
...
describe('selectableProjects', () => {
  test('represents projectless tasks as one choice while preserving the selected workspace', () => {
    const ordinary = project('repo', 'helm', '/srv/helm')
    const first = project('one', 'No project', '/home/me/.helm/projects/one')
    const selected = project('two', 'No project', '/home/me/.helm/projects/two')

...
How Helm talks to each coding agent: the process it launches, the wire protocol
...
polled off disk, and the one Helm generates itself — is in
[titles.md](titles.md).

Every provider is reached through the same driver abstraction in
[driver/mod.rs](../crates/helm-core/src/driver/mod.rs). There are seven
...
| --- | --- | --- |
| Codex app-server (JSON-RPC over stdio) | [driver/codex.rs](../crates/helm-core/src/driver/codex.rs) | Codex CLI |
| Agent Client Protocol (JSON-RPC over stdio) | [driver/acp.rs](../crates/helm-core/src/driver/acp.rs) | Cursor CLI, Fx, Grok Build, Kimi Code |
| OpenCode server (HTTP + server-sent events) | [driver/opencode.rs](../crates/helm-core/src/driver/opencode.rs) | OpenCode |
| Pi RPC mode (NDJSON request/response over stdio) | [driver/pi.rs](../crates/helm-core/src/driver/pi.rs) | Pi, Oh My Pi |
| Claude streaming-input session (NDJSON over stdio) | [driver/claude.rs](../crates/helm-core/src/driver/claude.rs) | Claude Code |
| Amp streaming-JSON session (NDJSON over stdio) | [driver/amp.rs](../crates/helm-core/src/driver/amp.rs) | Amp |
| Harness client API (typed HTTP + downlink streams) | [driver/deepseek.rs](../crates/helm-core/src/driver/deepseek.rs) | DeepSeek Harness |
...
Inputs ([driver/mod.rs:67](../crates/helm-core/src/driver/mod.rs#L79)):
...
Outputs ([model.rs:973](../crates/helm-core/src/model.rs)): `Connected`,
...
[driver/activity.rs](../crates/helm-core/src/driver/activity.rs), so the transcript renders
...
`Helm::runtimes` keyed by session id. Runtimes are per session, not per view:
...
([opencode_session.rs](../crates/helm-core/src/opencode_session.rs)). Helm quitting without
...
| Interactive approvals | yes | no | no (has them; Helm runs `--yolo`) | yes | no | yes | yes | yes | yes | yes |
...
([driver/codex.rs:164](../crates/helm-core/src/driver/codex.rs#L164)), plus `-c` config
overrides when Computer Use is on.

**Protocol** — newline-delimited JSON-RPC over stdio, genuinely bidirectional:
Codex can send Helm requests (approvals) and Helm answers them by id. Three
...
2. `initialized`.
3. `skills/extraRoots/set` when Computer Use is on, so Helm's bundled skill is
...
**Inbound stream** ([driver/codex.rs:851](../crates/helm-core/src/driver/codex.rs#L882)):
...
per-peer, the reader only treats method-less messages as replies to Helm's own
requests ([driver/codex.rs:779](../crates/helm-core/src/driver/codex.rs#L809)).
...
([driver/codex.rs:660](../crates/helm-core/src/driver/codex.rs#L690)).

**Models** — a throwaway app-server, `model/list` paged via `nextCursor`, up to
32 pages ([model_catalog.rs:367](../crates/helm-core/src/model_catalog.rs#L367)).

**Computer Use** — `-c mcp_servers.helm_js_repl.command=…` registers Helm's
QuickJS MCP server, with several `-c` flags disabling Codex's own external
computer-use plugin/MCP/skill so only Helm's `js` / `js_reset` surface is
visible.
...
([pi.rs:39](../crates/helm-core/src/driver/pi.rs#L39)) carries every divergence,
...
| Whole-session copy | in place | only at launch, so Helm shells out (see below) |
| Computer Use | Helm's Pi extension | none — Oh My Pi ships its own `/computer` |
...
`omp --mode rpc --yolo`
([pi.rs:246](../crates/helm-core/src/driver/pi.rs#L246)). Oh My Pi negotiates
...
Helm stamps each request with a string id (`helm-<n>`) and Pi answers with
...
with a 10 s timeout ([pi.rs:800](../crates/helm-core/src/driver/pi.rs#L800));
...
**Inbound stream** ([pi.rs:1182](../crates/helm-core/src/driver/pi.rs#L1182)):
...
| `extension_ui_request` | auto-cancelled — Helm has no UI for extension prompts |
...
access only" ([pi.rs:209](../crates/helm-core/src/driver/pi.rs#L209)).
Pi has no permission system at all, so `--approve` is the whole story. Oh My Pi
*does* have one, which Helm's `--yolo` then bypasses — the restriction is Helm's
...
([pi.rs:996](../crates/helm-core/src/driver/pi.rs#L996)). Rewind adopts the fork
...
performs in place. Oh My Pi only copies at launch, so Helm shells out to a
throwaway `omp --mode rpc --yolo --fork <session file>` and reads the new cursor
off it ([pi.rs:1108](../crates/helm-core/src/driver/pi.rs#L1108)). That is the
...
([model_catalog.rs](../crates/helm-core/src/model_catalog.rs)).

**Computer Use** — Pi only: `--extension <helm pi extension>` and
`--skill <SKILL.md>`, with the REPL and helper paths passed through the
environment. Helm's bridge is written against Pi's extension API, and Oh My Pi
...
([driver/claude.rs](../crates/helm-core/src/driver/claude.rs)), plus `--model`, `--effort`,
...
| `user` with `isReplay: true` | ignored — Helm's own prompt echoed by `--replay-user-messages` |
| `result` | `TurnFinished` |
...
`permission_suggestions`. Helm answers with a `control_response` whose result is
...
**Native checkpoints** — after each turn Helm reads Claude's own transcript at
...
`provider_resume_at` ([claude_session.rs](../crates/helm-core/src/claude_session.rs)). That
...
with fresh uuids; the returned id map is applied to Helm's retained turns.
...
routes resolved through CC Switch. Helm probes it in the background and caches
the last successful catalog; the curated list is only the startup/failure
fallback ([model_catalog.rs](../crates/helm-core/src/model_catalog.rs)).
...
([driver/amp.rs](../crates/helm-core/src/driver/amp.rs)). `--stream-json-thinking` implies
...
permission request on its stream; its rules live in `amp permissions`, so Helm
...
**Branch** — `amp threads export <id>` dumps the thread, Helm keeps the retained
...
(`HELM_AMP_BRANCH_CONTEXT_V1`). Forking a thread that was itself seeded this way
re-expands the nested envelope first, so branches of branches stay flat
([amp_session.rs](../crates/helm-core/src/amp_session.rs)).
...
([driver/opencode.rs](../crates/helm-core/src/driver/opencode.rs)). Helm already started this
...
only under `/api`, and since this server is Helm's alone, filtering by
...
([opencode_session.rs](../crates/helm-core/src/opencode_session.rs)).
...
([driver/acp.rs](../crates/helm-core/src/driver/acp.rs)).
...
to. Alongside Codex's app-server, this is the only transport where Helm's
...
capability, since Helm does not proxy the agent's file or terminal access — an
...
`session/resume` return provider-owned `configOptions`. Helm resolves the CLI's
...
an older Cursor agent advertises no model option, Helm retains the legacy
...
Helm first selects Fx's `gateway` provider option and reads the refreshed model
option from that response. Helm then targets the exact `model` id with
...
[kimi_session.rs](../crates/helm-core/src/kimi_session.rs) reads it, and
...
| `user_message_chunk` | ignored — Helm's own prompt echoed back |
...
than assistant content, so Helm filters that prelude from the transcript.
...
whole basis for the user's decision. Outside Supervised, Helm answers for the
...
services *Helm offers the agent*, not permissions the agent needs. `fs` exists so
an editor can serve unsaved buffer contents in place of what is on disk, and
`terminal` lets the agent run commands through the client's own terminal. Helm
...
Advertising a capability Helm cannot service is the harmful choice, because the
...
The one case that would justify serving `fs/read_text_file` is Helm's own file
...
**Access modes** — Fx exposes native `ask` and `code` modes, so Helm maps
Supervised to `ask` and the auto modes to `code`. Every other ACP agent stays in
its ordinary execution mode, and `auto_approve` decides whether Helm answers
...
name**, and the two disagree: Helm sends `mode` by default, but Kimi's `mode` is
...
advertise steering. Follow-ups remain in Helm's queue and start after the
...
([grok_session.rs](../crates/helm-core/src/grok_session.rs)), Cursor re-seeds a
fresh session ([cursor_session.rs](../crates/helm-core/src/cursor_session.rs)).
...
Helm's `RuntimeMode` (Supervised / Auto-accept edits / Auto / Full access)
maps into each CLI's own vocabulary.

| Helm | Codex (`approvalPolicy` / `sandbox` / reviewer) | Claude `--permission-mode` | Cursor | Fx | OpenCode | Grok | Kimi Code |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
`--yolo` bypasses the one it has. Only the last of those is Helm's own
limitation rather than the CLI's.

## Resume cursors

`ProviderResumeCursor` ([model.rs](../crates/helm-protocol/src/model.rs)) is
persisted with the session and is what makes a Helm task outlive its process:
...
| Provider | T3 Code transport | Helm transport |
| --- | --- | --- |
...
streaming-input protocol, which Helm speaks directly. No Node sidecar and no npm
dependency.

Helm goes one further than the comparison: Amp and Pi, which T3 Code does not
support, are long-lived here too. Every provider holds a session.

What the long-lived session buys, and what Helm pays for not having it:

| Capability | T3 Code | Helm |
| --- | --- | --- |
...
`capabilities` record. Helm's equivalent surface is split between
...
Note the parts that are *not* a gap. Helm's Codex path is the same app-server
...
1. Add the variant to `ProviderKind`
   ([model.rs](../crates/helm-protocol/src/model.rs)) with `id`,
...
3. Pick a transport, and look hard before settling for the one-shot path. Ask
   whether the CLI speaks ACP (`acp` / `agent stdio` — [driver/acp.rs](../crates/helm-core/src/driver/acp.rs)

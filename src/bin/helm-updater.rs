...
fn main() {
    eprintln!("helm-updater is only supported on Linux");
    std::process::exit(1);
}
...
mod linux {
    ...
    const MANAGED_MARKER: &str = "share/helm/self-update-v1";
    const MANAGED_MARKER_CONTENTS: &str = "helm-self-update-v1\n";
    const HELPER_EXECUTABLE: &str = "helm-updater";
    const RELAUNCH_READY_ENV: &str = "HELM_UPDATE_READY_FILE";
    ...
    impl Handoff {
        fn validate(arguments: Arguments) -> anyhow::Result<Self> {
            ...
            if arguments.parent_pid != unsafe { libc::getppid() as u32 } {
                bail!("the update helper was not launched directly by Helm");
            }
            ...
            {
                bail!("the staged install name does not belong to this Helm prefix");
            }
    ...
    fn apply_update(handoff: &Handoff) -> anyhow::Result<()> {
        ...
        if let Err(error) = fs::rename(&handoff.install_dir, &backup) {
            let message = format!("could not preserve the current Helm install: {error}");
        ...
        if let Err(error) = fs::rename(&handoff.staged_dir, &handoff.install_dir) {
            ...
            sync_directory(&handoff.parent);
            let message = format!("could not activate the staged Helm install: {error}");
            record_error(handoff, &message);
            let _ = launch(&handoff.install_dir, None);
            bail!(message);
        }
        ...
        let mut replacement = match launch(&handoff.install_dir, Some(&ready_path)) {
            Ok(child) => child,
            Err(error) => {
                let message = format!("the updated Helm build could not start: {error}");
                rollback(handoff, &backup, &message)?;
                bail!(message);
            }
        };
        ...
        match wait_for_relaunch(&mut replacement, &ready_path) {
            RelaunchState::Ready => {
                ...
                if let Err(error) = fs::remove_dir_all(&backup) {
                    eprintln!(
                        "Helm updater: update succeeded but the rollback copy {} could not be removed: {error}",
                        backup.display()
                    );
                }
                sync_directory(&handoff.parent);
            ...
            RelaunchState::Exited(status) => {
                let message =
                    format!("the updated Helm build exited before its window opened ({status})");
                rollback(handoff, &backup, &message)?;
                bail!(message)
            }
            RelaunchState::TimedOut => {
                ...
                eprintln!(
                    "Helm updater: the new build stayed alive but did not acknowledge startup; retaining {}",
                    backup.display()
                );
                Ok(())
            }
    ...
    fn rollback(handoff: &Handoff, backup: &Path, message: &str) -> anyhow::Result<()> {
        ...
        if let Err(error) = fs::rename(backup, &handoff.install_dir) {
            ...
            return Err(error).context("could not restore the previous Helm install");
        }
        sync_directory(&handoff.parent);
        let _ = fs::remove_dir_all(&failed);
        record_error(handoff, message);
        launch(&handoff.install_dir, None)
            .context("the previous Helm build was restored but could not be relaunched")?;
        Ok(())
    }

    fn launch(prefix: &Path, ready_path: Option<&Path>) -> std::io::Result<Child> {
        let mut command = Command::new(prefix.join("bin/helm"));
        command
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null());
    ...
    fn wait_for_relaunch(child: &mut Child, ready_path: &Path) -> RelaunchState {
        ...
        loop {
            ...
            match child.try_wait() {
                ...
                Ok(None) => {}
                Err(error) => {
                    eprintln!("Helm updater: could not observe the relaunched app: {error}");
    ...
    fn validate_packaged_layout(prefix: &Path) -> anyhow::Result<()> {
        ...
        if !metadata.file_type().is_dir() {
            bail!("the Helm prefix is not a real directory");
        }
        ...
        {
            bail!("the install is not marked as a Helm-managed tarball");
        }
        for executable in ["helm", "helm-daemon", HELPER_EXECUTABLE] {
    ...
    mod tests {
        ...
        fn write_layout(prefix: &Path, value: &[u8]) {
            ...
            fs::create_dir_all(prefix.join("share/helm")).unwrap();
        ...
        fn packaged_layout_requires_real_executables_and_marker() {
            ...
            let prefix = directory.join("helm.app");
            write_layout(&prefix, b"old");
            validate_packaged_layout(&prefix).unwrap();
            fs::remove_file(prefix.join("bin/helm-daemon")).unwrap();
        ...
        fn directory_swap_can_be_rolled_back_without_merging() {
            ...
            let install = directory.join("helm.app");
            let staged = directory.join(".helm.app.update-test");
            let backup = directory.join(".helm.app.backup-test");
            write_layout(&install, b"old");
            write_layout(&staged, b"new");

            fs::rename(&install, &backup).unwrap();
            fs::rename(&staged, &install).unwrap();
            assert_eq!(fs::read(install.join("bin/helm")).unwrap(), b"new");
            let failed = directory.join(".helm.app.failed-test");
            fs::rename(&install, &failed).unwrap();
            fs::rename(&backup, &install).unwrap();
            assert_eq!(fs::read(install.join("bin/helm")).unwrap(), b"old");
        ...
        fn full_handoff_waits_for_startup_and_removes_the_rollback_copy() {
            ...
            write_layout(&install, b"old");
            write_layout(&staged, b"new");
            let replacement = staged.join("bin/helm");
            fs::write(
                &replacement,
                b"#!/bin/sh\nprintf 'ready\\n' > \"$HELM_UPDATE_READY_FILE\"\n",
            )
            .unwrap();
            ...
            let handoff = Handoff {
                ...
                prefix_name: "helm.app".into(),
                parent_pid: std::process::id(),
            };

            apply_update(&handoff).unwrap();

            assert!(install.join("bin/helm").is_file());
            assert_eq!(fs::read(install.join("bin/helm-daemon")).unwrap(), b"new");
            assert!(

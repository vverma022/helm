...
impl Helm {
    ...
    fn open_command_palette_resume_view(
        ...
        cx.spawn(async move |helm, cx| {
            ...
            let _ = helm.update(cx, |helm, cx| {
                if !helm.command_palette.open
                    || helm.command_palette.provider_session_generation != generation
                    || helm.command_palette.resume_provider != provider
                {
                    return;
                }
                helm.command_palette.provider_sessions_pending = false;
                match result {
                    Ok(sessions) => {
                        helm.command_palette.provider_sessions = sessions;
                        helm.command_palette.provider_session_error = None;
                    }
                    Err(error) => {
                        helm.command_palette.provider_sessions.clear();
                        helm.command_palette.provider_session_error = Some(error.to_string());
                    }
                }
                if helm.command_palette.view == CommandPaletteView::Resume {
                    let query = helm.command_palette.search.read(cx).content().to_owned();
                    helm.refresh_command_palette_results(&query, false, cx);
                }
                cx.notify();
            });
        })
    ...
    fn load_command_palette_provider_session(
        ...
        cx.spawn(async move |helm, cx| {
            let result = cx.background_executor().spawn(async move { fetch() }).await;
            let update = helm.update(cx, |helm, cx| {
                if !helm.command_palette.open
                    || helm.command_palette.view != CommandPaletteView::Resume
                    || helm.command_palette.provider_session_generation != generation
                    || helm
                        .command_palette
                ...
                match result {
                    ...
                    Err(error) => {
                        ...
                        helm.command_palette.provider_session_import = None;
                        helm.command_palette.provider_session_error = Some(error.clone());
                        helm.show_toast(tr!("command_palette.resume_failed", error = error));
                        cx.notify();
                        None
                    }
                }
            ...
            let _ = window_handle.update(cx, |_, window, cx| {
                let _ = helm.update(cx, |helm, cx| {
                    if helm.command_palette.open
                        ...
                        && helm.command_palette.provider_session_generation == generation
                    {
                        helm.import_provider_session(summary, history, window, cx);
                    }
                });
            });
        })

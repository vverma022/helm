...
impl Helm {
    ...
    pub(super) fn ensure_session_loaded(&mut self, session_id: Uuid, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |helm, cx| {
            let result = cx
                .background_executor()
                .spawn(async move {
                    match helm_client::persistence::hydrate_session(&daemon, session_id)? {
            ...
            let _ = helm.update(cx, |helm, cx| {
                helm.session_hydrations.remove(&session_id);
                match result {
                    Ok(session) => {
                        let replaced = if let Some(existing) = helm
                            .state
                            .sessions
                            .iter_mut()
                        ...
                        if pending.is_some() {
                            helm.pending_session_activation = None;
                        }
                        if replaced && let Some(pending) = pending {
                            helm.finish_session_activation(session_id, pending.transition, cx);
                        } else if helm.state.selected_session == Some(session_id) {
                            helm.reset_visible_state();
                            helm.reset_transcript_rows(helm.transcript_row_count());
                            helm.refresh_composer_sources(cx);
                        }
                    }
                    Err(error) => {
                        if helm
                        ...
                        helm.show_toast(tr!("errors.open_session", error = error));
                    }
                }
                cx.notify();
            });
    ...
    pub(super) fn remove_session(&mut self, session_id: Uuid, cx: &mut Context<Self>) {
        ...
        if let Some(project_path) = project_path {
            ...
            cx.background_executor()
                .spawn(async move {
                    let _ = workspace.request(helm_client::WorkspaceOperation::DeleteSessionRefs {
                        cwd: project_path,
                        session_id,
                    });
                })
    ...
    /// [`Helm::sidebar_rendered_width`] / [`Helm::right_panel_rendered_width`].
    ...
    pub(super) fn cancel_turn(&mut self, cx: &mut Context<Self>) {
        ...
        // A provider runtime owns its Helm JavaScript REPL and Computer Use
        ...
        // remain resident so Helm can keep observing and stopping it.
        ...
        cx.spawn(async move |helm, cx| {
            let result = cx
                ...
                .spawn(async move {
                    match workspace.request(
                        helm_client::WorkspaceOperation::CreateProjectlessWorkspace {
                            prompt: None,
                        },
                    )? {
                        helm_client::WorkspaceResult::ProjectlessWorkspace { cwd } => Ok(cwd),
            ...
            let _ = helm.update(cx, |helm, cx| match result {
                Ok(cwd) => {
                    ...
                    let project_id = project.id;
                    helm.state.projects.push(project);
                    helm.create_session_for(project_id, helm.state.last_provider, cx);
                }
                Err(error) => {
                    helm.show_toast(tr!("errors.create_projectless_task", error = error));
                    cx.notify();
                }
            });
        })

...
pub(super) fn render_message_footer(
    ...
    helm: gpui::WeakEntity<Helm>,
) -> AnyElement {
    let theme = *theme;
    let message_id = message.id;
    let copy_helm = helm.clone();
    ...
    let copy_button = div()
        ...
        .on_click(move |_, _, cx| {
            ...
            let _ = copy_helm.update(cx, |this, cx| {
        ...
        if let Some(action) = assistant_message_action {
            ...
            footer = footer.child(if action.enabled {
                fork_button
                    .hover(|element| element.bg(theme.overlay_strong))
                    .on_click(move |_, _, cx| {
                        let _ = fork_helm.update(cx, |this, cx| {
        ...
        footer = footer.child(
            div()
                ...
                .on_click(move |_, window, cx| {
                    let _ = edit_helm.update(cx, |this, cx| {
...
pub(super) struct MessageRender<'a> {
    ...
    /// A row is laid out while the root `Helm` entity is already updating, so
    ...
    pub(super) helm: gpui::WeakEntity<Helm>,
    ...
    for (index, attachment) in attachments.iter().enumerate() {
        ...
        if attachment.is_image {
            ...
            if let Some(attachment_image) = attachment_image.as_ref() {
                let preview_helm = helm.clone();
                let key_helm = helm.clone();
                ...
                tile = tile.child(
                    div()
                        ...
                        .cursor_default()
                        .on_click(move |_, window, cx| {
                            let _ = preview_helm.update(cx, |this, cx| {
                ...
                tile = tile.on_key_down(move |event: &KeyDownEvent, window, cx| {
                    ...
                    if matches!(key, "enter" | "space") {
                        let _ = key_helm.update(cx, |this, cx| {
                            this.open_image_preview(
                                key_image.clone(),
                                key_name.clone(),
                                window,
    ...
    let element = match role {
        MessageRole::User => {
            ...
            if let Some(edit_input) = message_edit_input {
                ...
                let cancel_helm = helm.clone();
                let submit_helm = helm.clone();
                column = column.child(
                    div()
                        .w_full()
                        .max_w(px(540.0))
                            ...
                            div()
                                ...
                                .child(
                                    div()
                                        ...
                                        .on_click(move |_, window, cx| {
                                            let _ = cancel_helm.update(cx, |this, cx| {
                                ...
                                .child(
                                    div()
                                        ...
                                        .on_click(move |_, _, cx| {
                                            if can_submit {
                                                let _ = submit_helm.update(cx, |this, cx| {
...
mod message_time_tests {
    ...
    fn file_edit_title_and_stats_follow_the_activity_state() {
        ...
        .with_arguments(Some(
            serde_json::json!({
                "patch": "*** Begin Patch\n*** Update File: /tmp/helm/src/app.rs\n@@\n-old\n+new\n+more\n*** End Patch"
            })
            .to_string(),
        ));
    ...
    fn file_tool_titles_include_the_target_and_state() {
        ...
        .with_arguments(Some(
            serde_json::json!({"filePath": "/tmp/helm/src/model.rs"}).to_string(),
        ));
        ...
        .with_arguments(Some(
            serde_json::json!({"path": "/tmp/helm/src"}).to_string(),
        ));
    ...
    fn command_web_search_and_plan_titles_include_their_state() {
        ...
        .with_arguments(Some(serde_json::json!({"query": "Helm GPUI"}).to_string()));
        assert_eq!(
            activity_display_title(&web_search),
            "Searched the web for Helm GPUI"
        );

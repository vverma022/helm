    ...
    pub(super) fn schedule_composer_draft_save(&mut self, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |helm, cx| {
            cx.background_executor()
                ...
                .await;
            let Some((store, drafts)) = helm
                .update(cx, |helm, cx| {
                    if helm.composer_draft_save_generation != generation {
                        return None;
                    }
                    helm.capture_current_composer_draft(cx);
                    Some((
                        helm.composer_draft_store.clone(),
                        helm.composer_drafts.clone(),
                    ))
                })
                .ok()
                .flatten()
            ...
            if let Err(error) = result {
                let _ = helm.update(cx, |helm, cx| {
                    helm.show_toast(tr!("errors.save_local_state", error = error));
                    cx.notify();
                });
            }
        })

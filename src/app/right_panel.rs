...
/// Helm's compact editor, but it must not become part of the filesystem path.
...
fn review_diff_gap_icon_path(direction: crate::review_diff::ExpansionDirection) -> &'static str {
    match direction {
        ...
        // CSS. Helm names the data operation directly, so encode the resulting
...
/// only caller is [`Helm::read_right_panel_file_into_editor`].
fn read_right_panel_file(
    workspace: &helm_client::WorkspaceClient,
    project_path: &Path,
    relative_path: &str,
) -> (String, bool) {
    match workspace.request(helm_client::WorkspaceOperation::ReadTextFile {
        root: project_path.to_path_buf(),
        relative_path: PathBuf::from(relative_path),
    }) {
        Ok(helm_client::WorkspaceResult::TextFile { content }) => (content, true),
        Ok(_) => (
            tr!(
...
mod tests {
    ...
    fn working_tree_only_descends_into_expanded_directories() {
        let root = std::env::temp_dir().join(format!("helm-working-tree-{}", Uuid::new_v4()));
        std::fs::create_dir_all(root.join("src/nested")).unwrap();
        std::fs::create_dir_all(root.join(".git")).unwrap();
        std::fs::write(root.join("src/main.rs"), "fn main() {}\n").unwrap();
        std::fs::write(root.join("README.md"), "# Helm\n").unwrap();
        ...
        cx.spawn(async move |helm, cx| {
            ...
            helm.update(cx, |helm, cx| {
                        ...
                        .is_none_or(|path| path != project_path)
                {
                    ...
                    if let Some(editor) = helm.right_panel_file_editors.get_mut(&relative_path) {
                        editor.reading = false;
                    }
                    return;
                }
                let (content, writable) = read;
                let Some(editor) = helm.right_panel_file_editors.get_mut(&relative_path) else {
                    return;
                };
        ...
        cx.spawn(async move |helm, cx| {
            let result = cx
                ...
                .spawn({
                    ...
                    async move {
                        match workspace.request(helm_client::WorkspaceOperation::WriteTextFile {
                            root: project_path,
                            relative_path: PathBuf::from(relative_path),
                            content,
                        })? {
                ...
                cx.spawn(async move |helm, cx| {
                    let entries = cx
                        ...
                        .spawn({
                            ...
                            async move {
                                match workspace.request(helm_client::WorkspaceOperation::ListTree {
                                    root: path,
                                    expanded_paths: expanded.into_iter().collect(),
                                }) {
                                    Ok(helm_client::WorkspaceResult::WorkingTree { entries }) => {
                                        entries
                                            .into_iter()
                        ...
                        .await;
                    helm.update(cx, |helm, cx| {
                        if helm.working_trees.fulfill(token, entries.clone())
                            && helm
                                .selected_workspace_path()
                                .is_some_and(|path| path == project_path)
                        {
                            helm.right_panel_working_tree = entries;
                            cx.notify();
                        }
                    })
                    .ok();
    ...
    fn refresh_right_panel_diff(&mut self, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |helm, cx| {
            let result = cx
                ...
                .spawn({
                    ...
                    async move {
                        match workspace.request(
                            helm_client::WorkspaceOperation::CollectReviewDiff {
                                cwd: project_path,
                                source: crate::review_diff::wire_source(source),
                            },
                        )? {
                            helm_client::WorkspaceResult::ReviewDiff { data } => {
                ...
                .await;
            helm.update(cx, |helm, cx| {
                let still_current = helm.state.selected_session == Some(session_id)
                    && helm.right_panel_diff_generation == generation
                    && helm.right_panel_diff_source == source
                    && helm
                        .selected_workspace_path()
                        .is_some_and(|path| path == project_path);
                if !still_current {
                ...
                helm.right_panel_diff_loading = false;
                match result {
                    Ok(snapshot) => {
                        helm.right_panel_diff_selection.clear();
                        let directories = review_diff_directory_paths(&snapshot.files);
                        if had_snapshot {
                            helm.right_panel_diff_expanded_paths
                                .retain(|path| directories.contains(path));
                            helm.right_panel_diff_expanded_paths
                                .extend(directories.difference(&previous_directories).cloned());
                        } else {
                            helm.right_panel_diff_expanded_paths = directories;
                        }
                        helm.right_panel_diff_selected_file = selected_path
                            .as_deref()
                            .and_then(|path| {
                        ...
                        helm.right_panel_diff_snapshot = Some(Arc::new(snapshot));
                        helm.right_panel_diff_error = None;
                        helm.right_panel_diff_list_state.reset(line_count);
                        helm.sync_right_panel_diff_tree_rows(cx);
                    }
                    Err(error) => {
                        let message = error.to_string();
                        if helm.right_panel_diff_snapshot.is_some() {
                            helm.show_toast(tr!("diff.refresh_failed", error = message));
                        } else {
                            helm.right_panel_diff_error = Some(message);
                        }
                    }
                }
                cx.notify();

...
struct ConversationNavigationRailSnapshot {
    visible: bool,
    /// Shared with the `Helm` cache: the turns only change when the row-kinds
...
impl ConversationNavigationRail {
    ...
    pub(super) fn set_helm(&mut self, helm: WeakEntity<Helm>) {
        self.helm = Some(helm);
    }
    ...
    fn activate_turn(&self, message_id: Uuid, cx: &mut Context<Self>) {
        if let Some(helm) = &self.helm {
            let _ = helm.update(cx, |helm, cx| {
                helm.scroll_to_navigation_turn(message_id, cx)
            });
        }
    }
}
    ...
    fn prefetch_checkpoint_refs(&self, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |this, cx| {
            let existing = cx
                .background_executor()
                .spawn(async move {
                    match workspace.request(helm_client::WorkspaceOperation::SessionTurnRefs {
                        cwd: project_path,
                        session_id,
                    }) {
                        Ok(helm_client::WorkspaceResult::TurnRefs { turn_counts }) => {
                ...
                && (!sections.is_empty() || !activity.image_urls.is_empty())
            {
                ...
                for section in sections {
                    ...
                    if let Some(label) = section_kind.label() {
                        ...
                        let copy_helm = cx.entity().downgrade();
...
/// Pure window arithmetic behind [`Helm::live_reasoning_window_start`]:
...
fn render_activity_image(
    ...
    if helm_protocol::blob::is_reference(image_url)
        || image_url.starts_with(helm_protocol::attachments::ATTACHMENT_SCHEME)
    {
        return div()
            .id(id)
            .w(px(ACTIVITY_IMAGE_WIDTH))

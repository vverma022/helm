...
fn workspace_ack(
    ...
    operation: helm_client::WorkspaceOperation,
) -> anyhow::Result<()> {
    match workspace.request(operation)? {
        helm_client::WorkspaceResult::Ack => Ok(()),
...
fn workspace_has_ref(
    ...
    match workspace.request(helm_client::WorkspaceOperation::HasRef {
        cwd: cwd.to_path_buf(),
        git_ref: git_ref.to_owned(),
    })? {
        helm_client::WorkspaceResult::Bool { value } => Ok(value),
...
fn attach_driver(
    ...
    let Some(session) = helm_client::persistence::hydrate_session(&daemon, session_id)? else {
        return Ok(None);
    };
    let client = daemon.client();
    let response = client.request(session_id, Uuid::nil(), helm_client::Command::AttachSession)?;
    let helm_client::ResponsePayload::SessionRuntime {
        runtime_id,
        supports_steer,
    } = response
    else {
        anyhow::bail!("Helm daemon returned an invalid runtime attachment response");
    };
...
fn load_remote_task_state(
    ...
    let response = client.request(
        Uuid::nil(),
        Uuid::nil(),
        helm_client::Command::LoadTaskState,
    )?;
    let helm_client::ResponsePayload::TaskState {
        projects,
        mut sessions,
        ..
    } = response
    else {
        anyhow::bail!("Helm daemon returned an invalid task-state response");
    };
...
fn prepare_submission(
    workspace_client: helm_client::WorkspaceClient,
    project: Project,
    ...
    let workspace = match workspace {
        SessionWorkspace::NewWorktree { base_branch } => {
            ...
            let created =
                match workspace_client.request(helm_client::WorkspaceOperation::CreateWorktree {
                    ...
                    helm_client::WorkspaceResult::WorktreeCreated { worktree } => worktree,
    ...
    let checkpoint_warning = workspace_ack(
        &workspace_client,
        helm_client::WorkspaceOperation::CaptureTurnStart {
...
struct PreparedMessageRewind {
    ...
    claude_fork: Option<helm_client::provider_session::ProviderSessionFork>,
...
fn perform_message_rewind(
    ...
    let safety_ref = format!("refs/helm/revert-backup-{session_id}-{}", Uuid::new_v4());
    workspace_ack(
        &request.workspace_client,
        helm_client::WorkspaceOperation::CaptureRef {
    ...
    if let Err(error) = workspace_ack(
        &request.workspace_client,
        helm_client::WorkspaceOperation::RestoreRef {
            cwd: request.project_path.clone(),
        ...
        return Err(
            match workspace_ack(
                ...
                Ok(()) => {
                    let _ = workspace_ack(
                        &request.workspace_client,
                        helm_client::WorkspaceOperation::DeleteRef {
                            cwd: request.project_path.clone(),
                            git_ref: safety_ref.clone(),
                        },
                    );
    ...
    let cleanup_error = workspace_ack(
        &request.workspace_client,
        helm_client::WorkspaceOperation::DeleteTurnRefsAfter {
            cwd: request.project_path.clone(),
            session_id,
...
type ProviderRewindResult = (
    ...
    Option<helm_client::provider_session::ProviderSessionFork>,
    ...
    match provider {
        ProviderKind::Claude => {
            ...
            let fork = request.workspace_client.fork_provider_session(
                helm_client::provider_session::ProviderSessionForkRequest::Claude {
            ...
            let cursor = if let Some(driver) = request.driver.as_ref() {
                ...
                request
                    ...
                    .fork_provider_session(
                        helm_client::provider_session::ProviderSessionForkRequest::OpenCode {
        ...
        ProviderKind::Amp => {
            ...
            let cursor = request
                .workspace_client
                .fork_provider_session(
                    helm_client::provider_session::ProviderSessionForkRequest::Amp {
                        binary: binary.to_owned(),
                        cwd: request.project_path.clone(),
        ...
        ProviderKind::Cursor => {
            let source = request.cursor_source.as_ref().ok_or_else(|| {
                anyhow::anyhow!(tr!(
                    "errors.provider_helm_task_unavailable",
                    provider = "Cursor"
                ))
            })?;
            Ok((
                Some(
                    request
                        .workspace_client
                        .fork_provider_session(
                            helm_client::provider_session::ProviderSessionForkRequest::Cursor {
                                source: source.clone(),
        ...
        ProviderKind::Grok => {
            ...
            let cursor = request
                .workspace_client
                .fork_provider_session(
                    helm_client::provider_session::ProviderSessionForkRequest::Grok {
                        binary: binary.to_owned(),
                        cwd: request.project_path.clone(),
                        session_id: native_session_id.clone(),
                        turn_count: request.provider_turn_count,
...
fn perform_response_fork(mut request: ResponseForkRequest) -> Result<PreparedResponseFork, String> {
    ...
    let checkpoint_warning = workspace_ack(
        &request.workspace_client,
        helm_client::WorkspaceOperation::CopySessionRefs {
            cwd: request.source_workspace_path.clone(),
...
impl Helm {
    pub(super) fn restart_task_state_sync(&self) {
        ...
        std::thread::Builder::new()
            .name("helm-task-state-sync".into())
            .spawn(move || {
    ...
    pub(super) fn start_runtime_attachment(&mut self, session_id: Uuid, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |helm, cx| {
            let result = cx
                ...
                .await;
            let _ = helm.update(cx, move |helm, cx| {
                helm.finish_runtime_attachment(session_id, result, cx);
            });
        })
        .detach();
    }
                ...
                if *misses < 4 {
                    cx.spawn(async move |helm, cx| {
                        ...
                        let _ = helm.update(cx, |helm, cx| {
                            helm.start_runtime_attachment(session_id, cx);
                        });
                    })
                    .detach();
                } else {
    ...
    pub(super) fn request_provider_model_discovery(&mut self, provider: ProviderKind) {
        ...
        if std::thread::Builder::new()
            .name(format!("helm-{}-model-discovery", provider.id()))
            .spawn(move || {
                let discovered = match daemon.request(
                    Uuid::nil(),
                    Uuid::nil(),
                    helm_client::Command::ProbeProvider {
                        provider,
                        binary_override,
                    ...
                    Ok(helm_client::ResponsePayload::ProviderProbe { probe, .. }) => probe,
                    _ => probe,
                };
    ...
    /// contents can change while Helm stays open — models the user just
    ...
    pub(super) fn request_provider_version_probes(&mut self) {
        ...
        for provider in targets {
            ...
            let binary_override = self.state.provider_binary_overrides.get(&provider).cloned();
            if std::thread::Builder::new()
                .name(format!("helm-{}-version-probe", provider.id()))
                .spawn(move || {
                    let version = match daemon.request(
                        ...
                        Ok(helm_client::ResponsePayload::ProviderProbe { version, .. }) => version,
                        _ => None,
                    };
    ...
    pub(super) fn refresh_provider_detection(&mut self, scope: Option<ProviderKind>) {
        ...
        let daemon = self.daemon.client();
        if std::thread::Builder::new()
            .name("helm-provider-detection".into())
            .spawn(move || {
        ...
        for request in std::mem::take(&mut self.pending_checkpoint_captures) {
            ...
            cx.spawn(async move |helm, cx| {
                    ...
                    .spawn({
                        ...
                        async move {
                            match workspace.request(
                                helm_client::WorkspaceOperation::CaptureTurn {
                                    cwd: project_path,
                                    session_id,
                                    turn_count,
                                },
                            )? {
                                helm_client::WorkspaceResult::Checkpoint { checkpoint } => {
                                    Ok(checkpoint)
                                }
                                _ => anyhow::bail!(
                ...
                helm.update(cx, |helm, cx| {
                    helm.checkpoint_captures_in_flight
                        .remove(&(session_id, turn_count));
                    let selected = helm.state.selected_session == Some(session_id);
                    if selected {
                        helm.sync_transcript_rows();
                    }
                    let previous_kinds = if selected {
                        helm.transcript_row_kinds.borrow().clone()
                    } else {
                        Vec::new()
                    };
                    let checkpoint = match captured {
                        ...
                        Err(error) => {
                            helm.show_toast(tr!("errors.capture_turn_checkpoint", error = error));
                            Checkpoint {
                                turn_count,
                    ...
                    helm.invalidate_checkpoint_refs();
                    let mut attached_turn_id = None;
                    if let Some(session) = helm.state.session_mut(session_id)
                        ...
                        && selected
                    {
                        ...
                        helm.splice_transcript_rows_after_visibility_change(&previous_kinds);
                        helm.remeasure_changed_files(turn_id);
                    }
                    let resume_queue = helm.pending_queue_drains.contains(&session_id);
                    if resume_queue {
                        helm.pending_queue_drains.retain(|id| *id != session_id);
                        helm.drain_queued_message(session_id, cx);
                    }
                    cx.notify();
                    if attached_turn_id.is_some() {
                        ...
                        cx.spawn(async move |helm, cx| {
                            ...
                            let _ = helm.update(cx, |helm, _| helm.save());
                        })
                        .detach();
                    }
                })
    ...
    pub(super) fn fork_session_from_response(
        ...
        let request = ResponseForkRequest {
            workspace_client: helm_client::WorkspaceClient::new(self.daemon.client()),
            source,
        ...
        cx.spawn(async move |helm, cx| {
            let result = cx
                ...
                .await;
            let _ = helm.update(cx, move |helm, cx| {
                helm.finish_response_fork(session_id, turn_count, provider, result, cx);
            });
        })
        .detach();
    }
    ...
    pub(super) fn defer_restore_composer_after_fork(
        ...
        cx.spawn(async move |helm, cx| {
            cx.background_executor()
                ...
                .await;
            let _ = helm.update(cx, |helm, cx| {
                if helm.state.selected_session == Some(session_id) {
        ...
        cx.spawn(async move |helm, cx| {
            ...
            let _ = helm.update(cx, |helm, cx| {
                ...
                if is_current && !applied {
                    helm.reset_session_runtime(session_id);
                    cx.notify();
                }
            });
        })
    ...
    pub(super) fn start_goal_runtime(&mut self, session_id: Uuid, cx: &mut Context<Self>) {
        ...
        cx.notify();
        let workspace_client = helm_client::WorkspaceClient::new(self.daemon.client());
        cx.spawn(async move |helm, cx| {
            let prepared = cx
                .background_executor()
                .spawn(async move {
                ...
                .await;
            let _ = helm.update(cx, move |helm, cx| {
                helm.finish_goal_runtime_start(session_id, prepared, cx);
            });
        })
        .detach();
    }
    ...
    fn submit_submission_for_session(
        ...
        cx.spawn(async move |helm, cx| {
            let prepared = cx
                ...
                .await;
            let _ = helm.update(cx, move |helm, cx| {
                helm.finish_submission_preparation(session_id, submission, prepared, cx);
            });
        })
        .detach();
    }

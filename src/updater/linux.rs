//! Signed, rollback-safe updates for Helm's managed Linux tarball install.
//!
//! Checks, downloads, signature verification, and extraction all run on a
//! worker thread. Once an archive is fully staged, `helm-updater` validates
...
const FEED_URL: Option<&str> = Some("https://github.com/vverma022/helm/releases/latest/download/appcast-linux-aarch64.xml");
#[cfg(target_arch = "x86_64")]
const FEED_URL: Option<&str> = Some("https://github.com/vverma022/helm/releases/latest/download/appcast-linux-x86_64.xml");
...
impl InstallLayout {
    fn discover() -> Option<Self> {
        ...
        if executable.file_name()? != "helm" || executable.parent()?.file_name()? != "bin" {
            return None;
        }
...
impl Updater {
    ...
    pub fn install_available_update(&self) -> bool {
        ...
        if std::thread::Builder::new()
            .name("helm-updater-handoff".into())
            .spawn(move || {
...
fn extract_release_archive(path: &Path, destination: &Path, version: &str) -> anyhow::Result<()> {
    ...
    let expected_root = format!("helm-{version}-{triple}");
...
fn validate_packaged_layout(prefix: &Path) -> anyhow::Result<()> {
    ...
    anyhow::ensure!(
        ...
        "the install is not marked as a Helm-managed tarball"
    );
...
fn validate_download_url(value: &str) -> anyhow::Result<()> {
    ...
    anyhow::ensure!(
        url.scheme() == "https"
            ...
            && url.path().starts_with("/vverma022/helm/releases/")
            && url.username().is_empty()
            && url.password().is_none(),
        "the update feed points outside github.com/vverma022/helm/releases"
    );
    Ok(())
}
...
mod tests {
    ...
    fn extraction_strips_only_the_expected_release_root() {
        ...
        let root = format!("helm-9.8.7-{triple}");
        ...
        for (relative, contents, mode) in [
            ("bin/helm", b"app".as_slice(), 0o755),
            ("bin/helm-daemon", b"daemon".as_slice(), 0o755),
            ("bin/helm-updater", b"helper".as_slice(), 0o755),
        ...
        assert_eq!(fs::read(output.join("bin/helm")).unwrap(), b"app");

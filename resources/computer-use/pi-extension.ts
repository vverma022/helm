...
class HelmMcpClient {
  ...
  private async start(): Promise<void> {
    const executable = process.env.HELM_JS_REPL_SERVER;
    if (!executable) {
      throw new Error("HELM_JS_REPL_SERVER is not configured");
    }
    ...
    child.stderr.on("data", (chunk) => {
      ...
      if (message) console.error(`Helm JavaScript REPL: ${message}`);
    });
    child.once("error", (error) => this.handleExit(error));
    child.once("exit", (code, signal) => {
      this.handleExit(
        new Error(
          `Helm JavaScript REPL exited${code === null ? "" : ` with ${code}`}${
            signal ? ` (${signal})` : ""
          }`,
        ),
      );
    ...
    await this.requestWithoutStart("initialize", {
      ...
      capabilities: {},
      clientInfo: { name: "helm-pi", version: "1" },
    });
  ...
  private requestWithoutStart(method: string, params: JsonObject): Promise<unknown> {
    ...
    if (!child) return Promise.reject(new Error("Helm JavaScript REPL is not running"));
...
function toolResult(result: JsonObject) {
  ...
  if (result.isError === true) {
    const message = content
      ...
      .map((item) => item.text)
      .join("\n");
    throw new Error(message || "Helm JavaScript execution failed");
  }
  return { content, details: result._meta ?? {} };
}

export default function helmComputerUse(pi: ExtensionAPI) {
  const client = new HelmMcpClient();

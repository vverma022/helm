...
const appName = "Helm";
const executableName = "Helm";
const jsReplExecutableName = "helm_js_repl";
const daemonExecutableName = "helm-daemon";
const computerUseHelperName = "Helm Computer Use";
const packageName = "helm";
const defaultNotaryProfile = "NOTARY";
const projectRoot = resolve(import.meta.dir, "..");

const help = `Build, notarize, and publish a production release of Helm.
...
https://github.com/vverma022/helm/releases/latest/download/. One-time setup lives in RELEASING.md.

Options:
  --local                       Build, notarize, and write the DMG + zip
                                without publishing to R2
  --force                       Publish even if this version is already in R2
  --output <path>               DMG output path (default: dist/Helm-<version>.dmg)
  --signing-identity <name>     Developer ID Application identity selector
                                (or HELM_SIGNING_IDENTITY; required unless --adhoc)
  --notary-profile <name>       notarytool keychain profile
                                (default: NOTARY; or HELM_NOTARY_PROFILE)
  --build-number <number>       CFBundleVersion override (or HELM_BUILD_NUMBER;
                                default derives a monotonic number from the
                                Cargo version)
  --volume-name <name>          Mounted DMG name (default: Helm)
  --skip-build                  Reuse target/release/helm, helm_js_repl, and
                                helm-daemon
...
Environment:
  HELM_SIGNING_IDENTITY         Developer ID Application identity selector
  HELM_ANALYTICS_ENDPOINT       analytics endpoint embedded at build time
  HELM_ANALYTICS_WEBSITE_ID     analytics website ID embedded at build time
  HELM_R2_REMOTE                rclone remote name (default: r2)
  HELM_R2_BUCKET                R2 bucket name (default: helm-releases)
  HELM_DOWNLOAD_URL_PREFIX      base URL served by the bucket
                                (default: ${defaultDownloadUrlPrefix})
  HELM_HISTORY_COUNT            prior archives pulled for deltas (default: 15)
  HELM_NO_HISTORY=1             skip pulling prior archives (no deltas)
  SPARKLE_BIN                   Sparkle tools dir (default: the bundle.sh cache
                                under .helm-cache/sparkle)
...
const configuredSigningIdentity =
  values["signing-identity"] ?? process.env.HELM_SIGNING_IDENTITY;
const notaryProfile =
  values["notary-profile"] ??
  process.env.HELM_NOTARY_PROFILE ??
  defaultNotaryProfile;
const explicitBuildNumber =
  values["build-number"] ?? process.env.HELM_BUILD_NUMBER;
const analyticsEndpoint = process.env.HELM_ANALYTICS_ENDPOINT?.trim();
const analyticsWebsiteId = process.env.HELM_ANALYTICS_WEBSITE_ID?.trim();
...
const r2Remote = process.env.HELM_R2_REMOTE ?? "r2";
const r2Bucket = process.env.HELM_R2_BUCKET ?? "helm-releases";
...
const downloadUrlPrefix =
  process.env.HELM_DOWNLOAD_URL_PREFIX ?? defaultDownloadUrlPrefix;
const historyCount = Number(process.env.HELM_HISTORY_COUNT ?? "15");
const skipHistory = process.env.HELM_NO_HISTORY === "1";
...
if (!adhoc && !configuredSigningIdentity) {
  throw new Error(
    "Set HELM_SIGNING_IDENTITY or pass --signing-identity (or use --adhoc).",
  );
}
    ...
    if (detail.includes("directory not found")) {
      throw new Error(
        `R2 bucket "${r2Bucket}" does not exist on remote "${r2Remote}". ` +
          ...
          "github.com/vverma022/helm/releases custom domain (see RELEASING.md), then re-run.",
      );
    }
  ...
  const requests = [
    {
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "helm-release", version: "1" },
      },
    },
...
try {
  ...
  await $`env HELM_CODESIGN_IDENTITY=${identity} HELM_ANALYTICS_ENDPOINT=${analyticsEndpoint ?? ""} HELM_ANALYTICS_WEBSITE_ID=${analyticsWebsiteId ?? ""} HELM_SKIP_CARGO_BUILD=${values["skip-build"] ? "1" : "0"} ${join(projectRoot, "scripts", "bundle.sh")} release`;
  ...
  temporaryDirectory = await mkdtemp(join(tmpdir(), "helm-dmg-"));
  ...
  // archive as Helm-<version>.md; generate_appcast links it as the update's
  ...
  if (publishing) {
    ...
    console.log(`\nHelm ${version} (build ${buildNumber}) is live:`);

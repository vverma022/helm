...
// Sparkle-format feeds. Helm's native Linux updater reads this same compact
...
export async function generateLinuxAppcasts(
  ...
  for (const arch of architectures) {
    const archive = `helm-${version}-${targetTriple(arch)}.tar.gz`;
  ...
  if (written.length === 0) {
    throw new Error(`No helm-${version}-<target>.tar.gz found in ${assetsDir}`);
  }
  return written;
}

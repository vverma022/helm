...
//
// <assets-dir> holds this release's `Helm-<version>-<arch>-Setup.exe` files.
...
export async function generateWindowsAppcasts(
  ...
  for (const arch of architectures) {
    const installer = `Helm-${version}-${arch}-Setup.exe`;
  ...
  if (written.length === 0) {
    throw new Error(`No Helm-${version}-<arch>-Setup.exe found in ${assetsDir}`);
  }
  return written;
}

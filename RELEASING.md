...
Helm ships signed in-app updates on macOS, Linux, and Windows. Releases live in
a **Cloudflare R2** bucket served at **`https://github.com/vverma022/helm/releases/latest/download/`**. macOS uses
...
**This Mac already has the key** — Helm signs with the same default-account
...
Sparkle tools (they land in `.helm-cache/sparkle/<version>/bin` after any
...
To split Helm onto its own key later: `generate_keys --account helm`, put the
new public key in Info.plist, and pass `--account helm` through to
...
profile with `--notary-profile` / `HELM_NOTARY_PROFILE`.

### 3. Cloudflare R2 bucket + domain  ← **still to do once**

1. Create the bucket **`helm-releases`** (Cloudflare dashboard → R2 → Create
   bucket). The release script will not create it — a bucket-scoped API token
   can't.
2. Attach the custom domain **`github.com/vverma022/helm/releases`** to the bucket (bucket →
   Settings → Custom Domains). This serves objects publicly at
   `https://github.com/vverma022/helm/releases/latest/download/<file>`.
3. Make sure the R2 API token behind the `r2` rclone remote covers this bucket
   ...
   for kero; if `rclone lsf r2:helm-releases --s3-no-check-bucket` returns
...
- **Download link**: `https://github.com/vverma022/helm/releases/latest/download/Helm-<version>.dmg`
...
- `helm-<version>-x86_64-unknown-linux-gnu.tar.gz`
- `helm-<version>-aarch64-unknown-linux-gnu.tar.gz`
- `appcast-linux-x86_64.xml`, `appcast-linux-aarch64.xml`
- `latest-linux.txt` — the version `install.sh` resolves "latest" to

Windows CI adds:

- `Helm-<version>-x86_64-Setup.exe`
- `Helm-<version>-aarch64-Setup.exe`
- `helm-<version>-x86_64-pc-windows-msvc.zip` (portable)
- `helm-<version>-aarch64-pc-windows-msvc.zip` (portable)
...
[`resources/windows/helm.iss`](resources/windows/helm.iss) through Inno Setup's
`ISCC`. The installer is **per-user** (`PrivilegesRequired=lowest`,
`%LOCALAPPDATA%\Programs\Helm`) — no elevation, which is exactly what lets the
...
**Never change `AppId` in `helm.iss`.** It is how Windows recognizes an
...
prefix, then `helm-updater` swaps it after the app's normal quit saves and
...
distribution Helm can start on (2.35 — Ubuntu 22.04, Debian 12, Fedora 36).
...
`https://raw.githubusercontent.com/vverma022/helm/main/website/public/install.sh` — see [docs/linux.md](docs/linux.md).

Publishing that GitHub release (or running **Sync release** from Actions)
uploads the assets to the `helm-releases` R2 bucket. Configure these repository
secrets first:

| Secret | Purpose |
| --- | --- |
| `HELM_ANALYTICS_ENDPOINT` | embedded in every desktop CI build |
| `HELM_ANALYTICS_WEBSITE_ID` | embedded in every desktop CI build |
| `HELM_SIGNING_IDENTITY` | Developer ID identity selector |
...
| `R2_BUCKET` | optional; defaults to `helm-releases` |
...
| `--build-number <n>` / `HELM_BUILD_NUMBER` | derived | `CFBundleVersion` override |
| `HELM_R2_REMOTE` | `r2` | rclone remote name |
| `HELM_R2_BUCKET` | `helm-releases` | R2 bucket |
| `HELM_DOWNLOAD_URL_PREFIX` | `https://github.com/vverma022/helm/releases/latest/download/` | base URL in the appcast |
| `HELM_HISTORY_COUNT` | `15` | recent archives pulled for delta generation |
| `HELM_NO_HISTORY=1` | — | skip pulling old archives (full updates only) |
| `SPARKLE_BIN` | the `.helm-cache` copy | Sparkle tools directory |
...
- **Debug builds never update themselves.** `Updater::init` returns `None`
  ...
  itself with a production Helm. Set `HELM_FORCE_UPDATER=1` to exercise the
  ...
  start the watcher with `HELM_PREVIEW_UPDATE=1`; the sidebar immediately
...
- **Helm isn't sandboxed**, so Sparkle's XPC services are unnecessary;
...
- **Platform artifacts:** keep the bucket layout flat and platform-tagged by
  ...
  (`Helm-<v>.dmg`, `Helm-<v>.zip`, `appcast.xml`) must keep their URLs.
  Linux CI releases produce `helm-<v>-<target>.tar.gz` with
  `scripts/bundle-linux.sh`, Windows CI produces `helm-<v>-<target>.zip` with
  `scripts/bundle-windows.ts`, and both land in GitHub Releases, then R2 via
  the sync workflow. Windows also ships `Helm-<v>-<arch>-Setup.exe`; each
